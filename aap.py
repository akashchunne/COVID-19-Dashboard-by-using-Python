...
option2 = [{'label' : 'All', 'value' : 'Status'},
...
aap = dash.Dash(__name__, external_stylesheets=external_stylesheets)


aap.layout=html.Div([
    html.H1("Corona Virus Pandemic Analysis", style={'color' : 'white', 'text-align' : 'center'}),
    html.Div([
        html.Div([
            html.Div([
                html.Div([
...
@aap.callback(Output('bar','figure'),[Input('picker','value')])
def update_bar(type):
...
@aap.callback(Output('graph','figure'),[Input('plot-graph','value')])
def generate_graph(type):
    ...
    if type == 'Oxygen':
        return{'data': [go.Line(x=patients['Status'],y=patients['Oxygen'])],
               'layout':go.Layout(title='Commodities Total Count', plot_bgcolor='pink')}
@aap.callback(Output('the_graph','figure'),[Input('my_dropdown','value')])
